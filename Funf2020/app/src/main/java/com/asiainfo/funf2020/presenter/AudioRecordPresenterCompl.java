 ...
 * 502953057@qq.com
 */
public class AudioRecordPresenterCompl implements AudioRecordPresenter {

	private final static String TAG = AudioRecordPresenterCompl.class.getSimpleName();
	 ...
	 * 构造方法
	 *
	 * @param mAudioRecordView AudioRecordView
	 */
	public AudioRecordPresenterCompl(Context context, AudioRecordView mAudioRecordView) {
		this.mAudioRecordView = mAudioRecordView;
	...
	public void listenAudioRecording() {
		...
		final Handler checkHandler = new Handler() {
			...
			public void handleMessage(Message msg) {
				switch (msg.what) {
					case MSG_RUNNING:
						mAudioRecordView.onListenRecording(true);
						break;
					case MSG_STOP:
						mAudioRecordView.onListenRecording(false);
						break;
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				while (!mAudioRecordView.isAtBack()) {
					// 休眠一秒
					try {
	 ...
	 */
	@Override
	public void listenSDCard(final Context context) {
				...
				switch (msg.what) {
					case MSG_CALCULATE:
						String text = String.format(context.getString(R.string.clear_cache1),
								msg.obj + "MB");
						mAudioRecordView.setSDCardButtonText(text);
						break;
	...
	private void startRecord(Context context) {
		...
		mAudioRecordView.setScanNowButtonText(context.getString(R.string.stop));
	}
	...
	private void stopRecord(Context context) {
		...
		mAudioRecordView.setScanNowButtonText(context.getString(R.string.record));
	}
	 ...
	 */
	@Override
	public void doRecording(Context context) {
	...
	@Override
	public void bindRecordingService(Context context) {
		if (isRecordingRunning(context))
			context.bindService(intent, audioServiceConn, Context.BIND_AUTO_CREATE);
	}

	@Override
	public void unBindRecordingService(Context context) {
